...
<?php if ( bp_has_message_threads( bp_ajax_querystring( 'messages' ) ) ) : ?>
	...
	<form action="<?php echo bp_loggedin_user_domain() . bp_get_messages_slug() . '/' . bp_current_action() ?>/bulk-manage/" method="post" id="messages-bulk-management">
		...
		<table id="message-threads" class="messages-notices">
			...
			<thead>
				<tr>
					<th scope="col" class="thread-checkbox"><label class="bp-screen-reader-text" for="select-all-messages"><?php _e( 'Select all', 'buddyboss' ); ?></label><input id="select-all-messages" type="checkbox"></th>
					<th scope="col" class="thread-from"><?php _e( 'From', 'buddyboss' ); ?></th>
					<th scope="col" class="thread-info"><?php _e( 'Subject', 'buddyboss' ); ?></th>
					<th scope="col" class="thread-options"><?php _e( 'Actions', 'buddyboss' ); ?></th>
				</tr>
			</thead>
					...
					<tr id="m-<?php bp_message_thread_id(); ?>" class="<?php bp_message_css_class(); ?><?php if ( bp_message_thread_has_unread() ) : ?> unread<?php else: ?> read<?php endif; ?>">
						...
						<?php if ( 'sentbox' != bp_current_action() ) : ?>
							<td class="thread-from">
								<span class="from"><?php _e( 'From:', 'buddyboss' ); ?></span> <?php bp_message_thread_from(); ?>
								<?php bp_message_thread_total_and_unread_count(); ?>
								<span class="activity"><?php bp_message_thread_last_post_date(); ?></span>
							</td>
						<?php else: ?>
							<td class="thread-from">
								<span class="to"><?php _e( 'To:', 'buddyboss' ); ?></span> <?php bp_message_thread_to(); ?>
								<?php bp_message_thread_total_and_unread_count(); ?>
								<span class="activity"><?php bp_message_thread_last_post_date(); ?></span>
							</td>
						<?php endif; ?>
						...
						<td class="thread-options">
							<?php if ( bp_message_thread_has_unread() ) : ?>
								<a class="read" href="<?php bp_the_message_thread_mark_read_url();?>"><?php _e( 'Read', 'buddyboss' ); ?></a>
							<?php else : ?>
								<a class="unread" href="<?php bp_the_message_thread_mark_unread_url();?>"><?php _e( 'Unread', 'buddyboss' ); ?></a>
							<?php endif; ?>
							<a class="delete" href="<?php bp_message_thread_delete_link(); ?>"><?php _e( 'Delete', 'buddyboss' ); ?></a>
