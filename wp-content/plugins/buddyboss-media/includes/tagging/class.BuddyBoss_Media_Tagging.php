...
class BuddyBoss_Media_Tagging {
	...
	public function btn_tag_friends(){
		if( $this->can_tag_in_activity() ){
			?>
			<a href="#" class="button bp-secondary-action buddyboss_media_tag" onclick="return buddyboss_media_initiate_tagging(this);" data-activity_id="<?php bp_activity_id() ;?>">
	...
	public function ajax_get_tags(){
		...
		$friends_list = $this->get_friends_list( $activity_id );
		...
		if( $friends_list ){
			$retval['friends_list'] = "<ul>" . implode( "\n", $friends_list ) . "</ul>";
		} else {
			$retval['friends_list'] = "<div id='message'><p class='error'>" . __( 'You have no friends yet!', 'buddyboss-media' ) . "</p></div>";
		}
		
		die( json_encode( $retval ) );
	}
	
	protected function get_friends_list( $activity_id=false ){
		$friends = array();
