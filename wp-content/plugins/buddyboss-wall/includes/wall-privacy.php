...
function buddyboss_wall_get_visibility_lists($is_group_activity = false){
    ...
    $options = array(
        'public'	=> __('Everyone', 'buddyboss-wall'),
        'loggedin'	=> __('Logged In Users', 'buddyboss-wall'),
    );
...
function buddyboss_wall_last_activitymeta( $retval, $object_id, $meta_key ){
    if ($meta_key == 'bp_latest_update') {
		...
		if( !isset( $retval ) || empty( $retval ) ) { return false; }
 ...
 * @return type String
 */
function buddyboss_wall_editing_privacy_html() {
	if ( bp_is_group_home() ) {
		$options = buddyboss_wall_get_visibility_lists( true );
	} else {
		$options = buddyboss_wall_get_visibility_lists();
...
function buddyboss_wall_editing_privacy() {
	...
	if ( ( buddyboss_wall()->is_wall_privacy_enabled() ) && ( bp_get_activity_user_id() == bp_loggedin_user_id() ) ) {
		...
		?>	<a href="#" class="button bp-secondary-action buddyboss_privacy_filter <?php echo $apply_class; ?> " onclick="return buddyboss_wall_initiate_privacy_form(this);" data-activity_id="<?php bp_activity_id(); ?>" data-visibility="<?php echo $visibility; ?>">
