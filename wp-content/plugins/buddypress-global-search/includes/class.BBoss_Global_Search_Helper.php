				...
				if( !empty( $results ) ){
					...
					if( !empty( $this->search_results['all']['items'] ) && $args['template_type']!='ajax' ){
						...
						foreach( $ordered_items_group as $type=>&$items ){
							...
							$start_html = "<div class='results-group results-group-{$type} ". apply_filters( 'bboss_global_search_class_search_wrap', 'bboss-results-wrap', $type ) ."'>"
									.	"<h2 class='results-group-title'><span>" . apply_filters( 'bboss_global_search_label_search_type', $type ) . "</span></h2>"
									.	"<ul id='{$type}-stream' class='item-list {$type}-list ". apply_filters( 'bboss_global_search_class_search_list', 'bboss-results-list', $type ) ."'>";
